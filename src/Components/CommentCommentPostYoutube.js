...
class CommentCommentPostYoutube extends Component {
  _isMounted = false
  ...
  constructor(props) {
    ...
    this.fetchData_commentComment = this._fetchData_commentComment.bind(this) 
  }
  ...
  _fetchData_commentComment(accessToken) {   
    this.updateAccessToken(accessToken)  
    const requestId = this.requestId = uuidv1()

    commentComment(accessToken, this.props.commentId, this.state.text).then( responseJson => {
      if(this._isMounted && this.requestId === requestId) {                
        console.log("CommentPostYoutube :: _fetchData_commentComment :: responseJson :: " + JSON.stringify(responseJson))
  ...
  changeText = (text) => {        
    this.setState({text}, () => console.log('commentCommentPostYoutube :: changeText :: text :: ' + this.state.text))
  }
                ...
                onPress={() => {
                  if(this.state.text !== '' || !this.state.isLoading){
                    ...
                    this.setState({isLoading: true}, () => getAccessToken().then(accessToken => {
                                                             this.fetchData_commentComment(accessToken)
                                                           })) 
                  }
                }}
              >
...
export default withNavigation(CommentCommentPostYoutube)
