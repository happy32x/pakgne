...
class ModalEditTopCommentPostYoutube extends Component {
  _isMounted = false
  ...
  constructor(props) {    
    ...
    this.fetchData_editTopCommentFromApi = this._fetchData_editTopCommentFromApi.bind(this)
  }
  ...
  _fetchData_editTopCommentFromApi(accessToken) {   
    ...
    editTopCommentFromApi(accessToken, this.props.topCommentId, this.state.text).then( responseJson => {
      if(this._isMounted && this.requestId === requestId) {                
        ...
        if(responseJson.error && responseJson.error.code === 401) { //Invalid Credentials <> Access Token     
          getNewTokenFromApi_Filter(this.fetchData_editTopCommentFromApi)                          
        } else { //Success
  ...
  changeText = (text) => {        
    this.setState({text}, () => console.log('ModalEditTopCommentPostYoutube :: changeText :: text :: ' + this.state.text))
  }
          ...
          <View style={styles.comment_container_right}>
            ...
            <View style={styles.submit_button_container}>
              <BounceUpAndDownStatic
                scale={.8}
                onPress={() => {
                  if(this.state.text !== this.props.myText || !this.state.isLoading){
                    console.log('submit button pressed !')
                    this.setState({isLoading: true}, () => getAccessToken().then(accessToken => {
                                                             this.fetchData_editTopCommentFromApi(accessToken)
                                                           }))                     
                  }
                }}
              >
                {
                  this.state.text !== this.props.myText && !this.state.isLoading
...
export default withNavigation(ModalEditTopCommentPostYoutube)
